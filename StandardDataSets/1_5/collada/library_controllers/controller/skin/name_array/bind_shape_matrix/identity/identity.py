...
class SimpleJudgingObject:
    ...
    def JudgeSuperior(self, context):
        ...
        if ( self.__assistant.CompareRenderedImages(context) ):
            if ( not self.__assistant.CompareImagesAgainst(context, "_reference_1_influence", None, None, 5, False) ):
                context.Log ("PASSED: Image should not and does not match non_identity test case.")
                self.status_superior = True
                return self.status_superior
            else:
                context.Log ("FAILED: Image should not match non_identity test case.")
                self.status_superior = False
                return self.status_superior
