...
tagLst = ['library_controllers', 'controller', 'skin', 'source']
...
class SimpleJudgingObject:
            ...
            if len( Name_arrayInput ) != 0 and len( Name_arrayOutput ) != 0:
                ...
                if resSetEle[0] == True:            
                    ...
                    if resChkVale[0] == True:
                        testIO.Delink()
                        return [True, 'Pass: id of Name array is preserved.']
                    else:
                        testIO.Delink()
                        return [False, 'Failed: id of Name array is not preserved.']
                else:
                    testIO.Delink()
                    return [False, 'Failed: id of Name array is not found.']
            else:
                testIO.Delink()
                return [False, 'Failed: Name array is not found.']
        else:
            testIO.Delink()
    ...
    def JudgeBaseline(self, context):
        ...
        resCheckName = self.CheckName()
        
        self.msgB2M = resCheckName[1]
        
        if resCheckName[0] == True:            
            self.status_basic = 1
            return True
        else:
            self.status_basic = 0
