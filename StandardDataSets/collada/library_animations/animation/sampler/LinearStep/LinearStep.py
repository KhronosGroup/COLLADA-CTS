...
tagLst = ['library_animations', 'animation', 'source']
...
class SimpleJudgingObject:
    ...
    def NameArrayChecking(self):
        ...
        if len(sourceInputLst) != 0 and len(sourceOutputLst) != 0:
            ...
            if len(Name_arrayInput) != 0 and len(Name_arrayOutput) != 0:
                ...
                # check whether there is interpolaton retrived correctly
                if resSetEle[0] == True:            
                    resChkVale = testPChecker.checkValue(True)
                    if resChkVale[0] == True:                        
                        testIO.Delink()
                        return [True, 'Pass: interpolations in Name array are preserved.']
                    else:
                        testIO.Delink()
                        return [False, 'Failed: ' + resChkVale[1]]
                else:
                    testIO.Delink()
                    return [False, resSetEle[1]]
            else:
                testIO.Delink()
                return [False, 'Failed: Name array of import/export doesn\'t not existed.']
        else:
            testIO.Delink()
            return [False, 'Failed: source is not found or number of animation is not correct.']
    ...
    def JudgeBaseline(self, context):
        ...
        # Running function for Exemplary badge
        resNameCheck = self.NameArrayChecking()
        
        # record message:
        self.msgB2M = resNameCheck[1]
        
        if resNameCheck[0] == True:
            self.status_basic = 1
            return True  
        else:
            self.status_basic = 0
