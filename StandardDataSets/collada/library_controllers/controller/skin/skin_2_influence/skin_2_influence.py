...
class SimpleJudgingObject:
    ...
    def JudgeIntermediate(self, context):
        ...
        comparativeTestId = context.FindTestId( "reference_2_influence", "skin")
        if (comparativeTestId == None):
            context.Log("FAILED: You must also run the 'reference_2_influence' test case.")
            return False
        ...
        if (len(imageFilenames) == 0):
            context.Log("FAILED: The 'reference_2_influence' test must include a 'Render' step.")
            return False
        ...
        if result > 5:
            context.Log("FAILED: Output doesn't match the 'reference_2_influence' test. (" + str(result) + ")")
            return False
        else:
            context.Log("PASSED: Output matches the 'reference_2_influence' test. (" + str(result) + ")")
            return True
