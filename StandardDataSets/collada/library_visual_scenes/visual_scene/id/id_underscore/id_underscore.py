...
class SimpleJudgingObject:
    ...
    def AdditionBaselineChk(self):
        ...
        # get input visual_scene list
        visual_sceneInputLst = GetElementsByTags(testIO.GetRoot(self.inputFilleName), self.tagLst)
        visual_sceneOutputLst = GetElementsByTags(testIO.GetRoot(self.outputFilleNameLst[0]), self.tagLst)
        
        if visual_sceneInputLst != None and visual_sceneOutputLst != None:

            resSetEle = testPChecker.ResetElements(visual_sceneInputLst, visual_sceneOutputLst)
            ...
            if resSetEle[0] == True:            
                ...
                if resChkVale[0] == True:
                    testIO.Delink()
                    return [True, 'Pass: id of visual_scene is preserved.']
                else:
                    testIO.Delink()
                    return [False, 'Failed: id of visual_scene is not preserved.']
            else:
                testIO.Delink()
                return [False, 'Failed: id of visual_scene is not found.']
        else:
            testIO.Delink()
            return [False, 'Failed: visual_scene is not found.']
