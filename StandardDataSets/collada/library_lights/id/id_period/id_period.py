...
class SimpleJudgingObject:
    ...
    def AdditionExemplaryChk(self):
        ...
        # get input light list
        lightInputLst = GetElementsByTags(testIO.GetRoot(self.inputFilleName), self.tagLst)
        lightOutputLst = GetElementsByTags(testIO.GetRoot(self.outputFilleNameLst[0]), self.tagLst)
        
        if lightInputLst != None and lightOutputLst != None:

            resSetEle = testPChecker.ResetElements(lightInputLst, lightOutputLst)
            ...
            if resSetEle[0] == True:            
                ...
                if resChkVale[0] == True:
                    testIO.Delink()
                    return [True, 'Pass: id of light is preserved.']
                else:
                    testIO.Delink()
                    return [False, 'Failed: id of light is not preserved.']
            else:
                testIO.Delink()
                return [False, 'Failed: id of light is not found.']
        else:
            testIO.Delink()
            return [False, 'Failed: light is not found.']
