...
class TwoStepJudgingObject:
    ...
    def CachedBasic(self, context):
        ...
        comparativeTestId = context.FindTestId( "sphere_polylist", "aggregate" )
        if (comparativeTestId == None):
            logs.append("FAILED: You must also run the 'aggregate|sphere_polylist' test case.")
            return (logs, False)
        ...
        if (len(imageFilenames) == 0):
            logs.append("FAILED: The 'aggregate|sphere_polylist' test must include a 'Render' step.")
            return (logs, False)
        ...
        if result > 5:
            logs.append("FAILED: [Comparison: %d] Output doesn't match the 'aggregate|sphere_polylist' test. " % result)
            return (logs, False)
        else:
            logs.append("PASSED: [Comparison: %d] Output matches the 'aggregate|sphere_polylist' test. " % result)
            return (logs, True)
